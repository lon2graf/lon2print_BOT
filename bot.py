...
def get_difint():
	message_sender(id,"Напишите пожалуйста еще немного дополнительной информации, которая вам придет в голову.(Необходимость доставки, сроки и тд)")
	i = 0
	for event in longpoll.listen():
		if event.type == VkEventType.MESSAGE_NEW and event.to_me:
			msg = event.text
...
for event in longpoll.listen():
	if event.type == VkEventType.MESSAGE_NEW and event.to_me:
		...
		if (msg == "Начать") or (msg == "начать") or (msg == "start") or (msg == "Start") :
			...
			message_sender(id, "Меня создал и воспитал - https://vk.com/vanluv_lon2graf")
			...
			message_sender(id, "Прости я бот и понимаю лишь то что написано в моих исходниках. Пиши !чего_умеешь.")
